...
- **Download folder** zips the item's directory on the fly and streams it to you (`.incomplete` and hidden files are left out; the manifest goes in without its keys). Zips are capped at 4 GB; larger items offer their files one by one instead.
