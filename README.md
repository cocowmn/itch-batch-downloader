...
- **Download folder** — the item's whole directory as a zip, streamed while it is built (`.incomplete` and hidden files are left out; the manifest goes in without its keys). Zips are limited to 4 GB; above that the option disappears and the files can be downloaded one by one.
