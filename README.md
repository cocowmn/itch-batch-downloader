...
- **Download folder** — the item's whole directory as a zip, streamed while it is built (`.incomplete`, hidden and system files such as `.DS_Store` or `Thumbs.db` are left out; the manifest goes in without its keys). There is no size limit: archives past 4 GB are written in Zip64, which every current unzipper (Archive Utility, Explorer, 7-Zip, `unzip`) reads.
- **Download from itch.io** — fetches a fresh copy of the item through the downloader (same settings as a CLI run: files, cover, captures, videos, manifest) into a temporary directory on the server and hands it over as a zip; an overlay shows progress and can cancel. It needs the item's download page, which manifests only carry when `manifest_include_keys` is on, and the server needs `cookies.txt`. One download runs at a time (a selection of items counts as one: they are fetched one after the other into the same zip, a folder each, and one item failing does not stop the rest); the temporary files are removed after the zip is sent (or after ten minutes). The zip's manifest is always written without keys. Your library on disk is not touched. When both ways to download are available they share one button (folder icon with a chevron) that opens a small menu; when only one is, its icon button acts directly.
- **Selecting several items** — Shift+click an item (or press and hold it on a phone) to start a selection; from then on a click toggles an item, `⌘A` / `Ctrl+A` selects everything the filter shows, and `Esc` (or the × in the bar) clears it. A bar at the bottom counts the selection and its size and offers: *Copy details*, which puts a JSON array of the items' manifests (without keys; an item without a manifest contributes what the library knows about it) on the clipboard; *Download*, which follows the same rules as the item's button — a menu of *Download folders* and *Download from itch.io* when every selected item can be fetched again, the folder download alone otherwise — and asks for a name for the archive (`2026-09-12--itch.io-downloads` by default) before sending one zip with a folder per item; and, for admins, an eye button that hides the selection (or unhides it, once every item in it is hidden) and *Delete*.
...
The server serves nothing outside the download directory. The refresh button (or reloading the page) rescans the directory, so it can run while a download is in progress. Nothing is written unless an admin hides, deletes or unzips an item (below).

### Admin: hiding, deleting and unzipping items

`appconfig.toml` gets a random `admin_password` when it is created (change it to anything you like, or remove the line to turn the admin features off). **Click the library icon** at the left of the top bar and choose *Admin sign-in*; the session lasts until the tab or the server closes (the same menu offers *Admin session* to sign out again). Signed in, you get:
...
- **Unzip** — the icon of every `.zip` in the file list turns into an open-package button on hover: click it to extract the archive next to itself. *Unzip all* at the top of the details panel (only shown when the item folder has zips at its top level) extracts each of them, and the viewer's card for a zip has an *Unzip archive* button too. Extraction follows Archive Utility's rules: an archive holding a single file or folder produces just that, anything else lands in a folder named after the archive, `__MACOSX` sidecars are dropped, and nothing is ever overwritten — a name that exists gets ` (1)`, ` (2)`, … appended. Only stored and deflated entries are supported (what nearly every zip uses); other archive formats (`.rar`, `.7z`) cannot be extracted.
- **Delete item** in red at the bottom of the details panel — removes the item's folder from disk after a confirmation. This cannot be undone. With several items selected, the *Delete* button in the selection bar removes them all after one confirmation, and the eye button next to it hides them all (it turns into *unhide* when every selected item is hidden).
...
| `download_manifest` | `true` | Write `<item>_manifest.json` next to the files: title, author, page and download-page URLs, itch.io id, description, cover URL, the "More information" panel (status, category, genre, …), tags, screenshot URLs, embeds, the bundle(s) the item was selected from, and the files in the directory (hidden and system files such as `.DS_Store` are not listed). Refreshed on every run. |
